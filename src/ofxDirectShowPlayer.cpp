//DirectShowVideo and ofxDirectShowPlayer written by Theodore Watson, Jan 2014
...
class ofxDirectShowVideo : public ISampleGrabberCB{
    public:
...
ofxDirectShowPlayer::ofxDirectShowPlayer(){
    player = NULL;
}
...
bool ofxDirectShowPlayer::load(string path){
    path = ofToDataPath(path); 

    close();
    player = new ofxDirectShowVideo();
    bool loadOk = player->loadMovie(path);
    if( !loadOk ){
        ofLogError("ofxDirectShowPlayer") << " Cannot load video of this file type.  Make sure you have codecs installed on your system.  OF recommends the free K-Lite Codec pack. " << endl;
    }
    return loadOk;
}

void ofxDirectShowPlayer::close(){
    if( player ){
        delete player;
        player = NULL;
    }
...
void ofxDirectShowPlayer::update(){
...
void ofxDirectShowPlayer::play(){
    if( player && player->isLoaded() ){
        player->play();
    }
}

void ofxDirectShowPlayer::stop(){
    if( player && player->isLoaded() ){
        player->stop();
    }
}       
    
bool ofxDirectShowPlayer::isFrameNew() const{
    return ( player && player->isFrameNew() ); 
}

const ofPixels & ofxDirectShowPlayer::getPixels() const{
    return pix;
}

ofPixels & ofxDirectShowPlayer::getPixels(){
    return pix;
}

float ofxDirectShowPlayer::getWidth() const{
    if( player && player->isLoaded() ){
...
float ofxDirectShowPlayer::getHeight() const{
    if( player && player->isLoaded() ){
...
bool ofxDirectShowPlayer::isPaused() const{
    return ( player && player->isPaused() ); 
}

bool ofxDirectShowPlayer::isLoaded() const{
    return ( player && player->isLoaded() ); 
}

bool ofxDirectShowPlayer::isPlaying() const{
    return ( player && player->isPlaying() ); 
}   

bool ofxDirectShowPlayer::setPixelFormat(ofPixelFormat pixelFormat){
    return (pixelFormat == OF_PIXELS_RGBA);
}

ofPixelFormat ofxDirectShowPlayer::getPixelFormat() const{
    return OF_PIXELS_RGBA; 
}
        
//should implement!
float ofxDirectShowPlayer::getPosition() const{
    if( player && player->isLoaded() ){
...
float ofxDirectShowPlayer::getSpeed() const{
    if( player && player->isLoaded() ){
...
float ofxDirectShowPlayer::getDuration() const{
    if( player && player->isLoaded() ){
...
bool ofxDirectShowPlayer::getIsMovieDone() const{
    return ( player && player->isMovieDone() ); 
}
    
void ofxDirectShowPlayer::setPaused(bool bPause){
    if( player && player->isLoaded() ){
        player->setPaused(bPause);
    }
}

void ofxDirectShowPlayer::setPosition(float pct){
    if( player && player->isLoaded() ){
        player->setPosition(pct);
    }
}

void ofxDirectShowPlayer::setVolume(float volume){
    if( player && player->isLoaded() ){
        player->setVolume(volume);
    }
}

void ofxDirectShowPlayer::setLoopState(ofLoopType state){
    if( player ){
        if( state == OF_LOOP_NONE ){
            ...
            ofLogError("ofxDirectShowPlayer") << " cannot set loop of type palindrome " << endl;
        }
    }
}

void ofxDirectShowPlayer::setSpeed(float speed){
    if( player && player->isLoaded() ){
        player->setSpeed(speed); 
    }
}
    
int ofxDirectShowPlayer::getCurrentFrame() const{
    if( player && player->isLoaded() ){
...
int ofxDirectShowPlayer::getTotalNumFrames() const{
    if( player && player->isLoaded() ){
...
ofLoopType ofxDirectShowPlayer::getLoopState() const{
    if( player ){
        if( player->isLooping() ){
...
void ofxDirectShowPlayer::setFrame(int frame){
    if( player && player->isLoaded() ){
...
void ofxDirectShowPlayer::firstFrame(){
    setPosition(0.0); 
}

void ofxDirectShowPlayer::nextFrame(){
    if( player && player->isLoaded() ){
        player->nextFrame();
    }
}

void ofxDirectShowPlayer::previousFrame(){
    if( player && player->isLoaded() ){
        player->preFrame();
    }
}
